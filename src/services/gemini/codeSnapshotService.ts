...
2. **설계도 매핑 및 자동 부모(Task/Feature) 생성 (Step 2 & 2-1)**:
   ...
   - **엄격한 계층 규칙**:
     ...
     - **Reference**: 코드 로직의 최소 단위 구현체.
   - **부모 생성 규칙 (noteType 결정)**:
     - **Task (말단 작업)**: 특정 알고리즘, 정규표현식 처리, 데이터 포맷 변환 등 '어떻게(How)' 처리하는지에 집중된 원자적 코드 조각인 경우.
     - **Feature (기능 단위)**: 파일의 전체 목적을 대변하는 핵심 인터페이스, 복합적인 책임을 가진 로직(여러 Sub-tasks 제어), 또는 비즈니스 요구사항 명세서에서 하나의 '기능'으로 명명될 수 있는 무게감을 가진 경우.
   - **Sibling Promotion (중요)**: 만약 Task 아래에 또 다른 Task가 생겨야 할 정도로 로직이 복잡하다면, 현재 Task를 Feature로 승격시키고 새로운 Task를 그 아래에 두는 구조를 제안하십시오.
